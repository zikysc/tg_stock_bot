...
class StockAPI:
    ...
    async def get_kline_plot_data(self, stock_code: str, days: int = 120) -> List:
        ...
        try:
            r = await self.client.get(
                f"/api/v1/stocks/{stock_code}/history",
                params={"period": "daily", "days": str(days)}
            )
    ...
    async def analyze(self, code: str, name: str):
        await self.login_if_needed()
        payload = {
            "async_mode": False, "force_refresh": False, "notify": True,
            "original_query": name, "report_type": "detailed",
            "selection_source": "autocomplete", "stock_code": code, "stock_name": name
        }
